...
import time # added to solve name 'sleep' is not defined 
import numpy as np
...
def try_start_ray(local_mode):
    """ 
    Prefer connecting to an existing Ray head (address='auto'), helpful in the batch/srun allocation. Fallback to starting a local Ray runtime for quick interactive testing.

    """
    depth = 0
    while True:
        try:
            print("Trying ray.init(address='auto') to connect to existing head...")
            # this connects when a ray head is already started (recommended under slurm)
            ray.init(address="auto")
            print("Connected to existing Ray cluster.")
            break
        except Exception as e_auto:
            # try to start a local cluster if connecting fails (keeps interactive testing simple)
            try:
                print(f"No existing head found (error: {e_auto}). Trying ray.init(local_mode={local_mode}...")
                ray.init(local_mode=local_mode, include_dashboard=False)
                print("Started a local Ray runtime.")
                break
            except Exception as e_local:
                waittime = np.random.randint(1, 10 * 2**depth)
                print(f"Failed to start/connect Ray (auto err: {e_auto}; local err: {e_local}). Retrying in {waittime} s...")
                time.sleep(waittime)
                depth += 1
...
if __name__ == '__main__':
    ...
    # Try to connect Ray head first (recommended when running inside SLURM job 
    # that starts a ray head), otherwise fallback to local runtime for testing.
    ...
    #configure framework/env/callbacks/training etc.
    config.framework(
        framework='torch'
    ).environment(
        env=env_id,
    ...
    # --- Convert to dict & inject GPU resource requests for Ray ---
    # Make a config dict for Tuner that *explicitly* requests GPUs so Ray schedules them.
    cfg_dict = config.to_dict()

    # Set how many GPUs the driver (algorithm) should use:
    # - 1 means the driver (trainer) expects 1 GPU.
    # - Set to 0 if you want only workers to use GPUs.
    cfg_dict.setdefault('num_gpus', 1)

    # Set how many GPUs each worker (RolloutWorker) should use.
    # If you have only 1 GPU total and multiple workers, keep this at 0.0,
    # and let the driver/optimizer use the GPU (num_gpus=1).
    # If you want each worker to have a full GPU, set to 1.0 (but then
    # the cluster must have enough GPUs).
    cfg_dict.setdefault('num_gpus_per_worker', 0.0)

    # Older/newer RLlib variants may use other keys for GPU-per-policy; add if needed:
    # cfg_dict.setdefault('num_gpus_per_policy', {"default_policy": 1.0})

    # --- Keep the existing build/load-checkpoint flow (useful if you want to resume) ---
    algo = config.build()
    ...
    tuner = tune.Tuner(
        'SimpleQ',
        param_space=config.to_dict(),  # use the dict with explicit GPU requests
